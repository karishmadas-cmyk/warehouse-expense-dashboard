...
@st.cache_data
def load_data():
    # Read the Excel file (Header starts at line index 1 based on dataset)
...
selected_zone = st.sidebar.multiselect(
    "Select Zone", options=df["Zone"].unique(), default=df["Zone"].unique()
)
...
selected_loc = st.sidebar.multiselect(
    ...
    default=df["Locations"].unique(),
)

# Apply Filters
filtered_df = df[
    (df["Zone"].isin(selected_zone))
    & (df["Customer"].isin(selected_cust))
    & (df["Locations"].isin(selected_loc))
]

# Top KPI Metric Cards
total_exp = filtered_df["Total Expenses"].sum()
...
col_kpi4.metric(
    "Avg Expense / Line Item",
    f"₹ {filtered_df['Total Expenses'].mean() / 1e5:.2f} L",
)
...
with col1:
    ...
    m_trend = (
        filtered_df.groupby("Month_Str")["Total Expenses"].sum().reset_index()
    )
    fig1 = px.line(
        m_trend,
        x="Month_Str",
